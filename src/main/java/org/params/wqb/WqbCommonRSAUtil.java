...
import org.params.wqb.req.WqbReq;
...
//        String thirdPublicKey = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDssd0uSCRk/g7CePp9aBzemQ3Wzr8rdgSaQAahW4Nt30BBSLptDe/4+7mb2DYIjmNUYR3MKMeUtXriRTmN+avR4pz4ibQRdH1ARu91GhAfbzBDOROS8wtt+738TESUk3Nis24FP9pUF/ZwW1sHcKCxqt1JoIqDADaSg6SVPWSm5wIDAQAB";
//
//        // 己方的私钥，用于解密(测试环境默认都用一样的)
//        String selfPrivateKey = "";
//
//        // 请求明文
...
//    }
     ...
     * @param wqbConfigEnum 对方的公钥
     * @return 加密加签之后的请求体
     */
    public static WqbResp encryptionAndSignAndBuild(String requestData, WqbConfigEnum wqbConfigEnum) {
        log.info("加密 请求明文data:{}", requestData);
        try {
            String thirdPublicKey=wqbConfigEnum.getPublicKey();
            //获取随机密钥R
...
//            log.info("签名sign:{}", sign);
            WqbResp wqbResp = new WqbResp();
            wqbResp.setData(data);
            wqbResp.setSign(sign);
            wqbResp.setKey(key);
//            wqbResp.set("timestamp", System.currentTimeMillis());
            log.info("加密结束 requestJson:{}", JSONObject.toJSONString(wqbResp));
            return wqbResp;
        } catch (Exception e) {
     ...
     *
     * @param wqbReq    请求json
     * @param wqbConfigEnum 配置
     * @return 明文data
     */
    public static String decryptAndGetData(WqbReq wqbReq, WqbConfigEnum wqbConfigEnum) {
        log.info("解密 密文requestJson:{}", JSONObject.toJSONString(wqbReq));
        String data = wqbReq.getData();//密文data
        String sign = wqbReq.getSign();//签名sign
        String key = wqbReq.getKey();//加密密钥key
        if (StringUtils.isEmpty(data) || StringUtils.isEmpty(sign) || StringUtils.isEmpty(key)) {
            return null;
        }
        String selfPrivateKey=wqbConfigEnum.getPrivateKey();
        try {
...
//            log.info("对签名S进行解密得到摘要B:{}", abstractB);
    ...
    public static String decryptRSA(String content, String privateKey, String inputCharset) throws Exception {
        ...
        InputStream ins = new ByteArrayInputStream(Base64.decodeBase64(content.getBytes(StandardCharsets.UTF_8)));
        ByteArrayOutputStream writer = new ByteArrayOutputStream();
        int bufLength;
        byte[] block;
